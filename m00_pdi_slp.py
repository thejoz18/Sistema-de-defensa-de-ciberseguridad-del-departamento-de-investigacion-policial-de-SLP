##Importamos random para decidir si ocurre un ataque simulado.
import random
##Importamos time para pausar la simulacion entre ciclos.
import time

##Importamos los modulos propios del proyecto.
...
if respuesta_inicio == "si" or respuesta_inicio == "s":
    ...
    for segundo in range(1, 61):
        ...
        ##Guardamos la temperatura original antes de aplicar ventilador o refrigeracion.
        paquete["temperatura_sin_enfriamiento"] = paquete["temperatura"]

        ##Calculamos la senal normal esperada para comparar contra el trafico real.
        senal    = lm.senal_periodica(segundo)
        ##Comparamos los paquetes reales contra la senal esperada.
        ...
        ##Evaluamos el paquete para decidir estado, alertas, protecciones y cifrado.
        est, alertas, protecciones, cifrado = lm.evaluar_estado(
            paquete, flujo, estado_actual["cifrado"], indice_oscilatorio
        )
        ##Actualizamos el estado principal del sistema con el resultado calculado.
        estado_actual["estado"]        = est
        if est != "NORMAL":
            ##Guardamos el estado no normal para recordarlo al final
            estado_actual["estado_arrastrado"] = est
        ##Guardamos el nivel de cifrado que quedo activo en este ciclo.
        estado_actual["cifrado"]       = cifrado
        ##Copiamos el cifrado al paquete para que tambien aparezca en reportes.
        paquete["cifrado"] = estado_actual["cifrado"]

        ##Creamos la ruta interna normal o cifrada segun el estado de riesgo.
        paquete["direccion_registro"] = lm.generar_direccion_registro(
            paquete, estado_actual["estado"], estado_actual["cifrado"], segundo
        )
        ##Creamos el enlace visible del servidor para mostrarlo en consola y CSV.
        ...
        ##Calculamos derivadas de CPU y temperatura para analizar cambios fisicos.
        derivada_cpu, derivada_temperatura = lm.calcular_derivadas_fisicas(
            paquete, estado_calculo
        )

        ##Convertimos la refrigeracion a 1 o 0 para guardarla facil en el CSV.
        if paquete["refrigeracion_activa"]:
            refrigeracion_valor = 1
        else:
            refrigeracion_valor = 0

        ##Convertimos el incidente a 1 o 0 para que el reporte sea mas claro.
        if paquete["incidente"]:
            incidente_valor = 1
        else:
            incidente_valor = 0

        ##Guardamos un resumen completo del ciclo dentro del historial.
        ##Solo se comenta el diccionario completo para no repetir comentario en cada campo.
        ...
        ##Preparamos un texto corto para indicar si el ciclo tuvo incidente.
        inc_txt = ""
        if paquete["incidente"]:
            inc_txt = "  | " + paquete.get("tipo_incidente", "NINGUNO")
        ##Imprimimos una linea de monitoreo con los datos principales del ciclo.
        mon.imprimir_ciclo(segundo, paquete, estado_actual["estado"], inc_txt)

        ##Cada 5 segundos se imprime un panel mas completo del estado del sistema.
        if segundo % 5 == 0:
            ##Tomamos los ultimos 5 ciclos para sacar promedios recientes.
            ultimos_cinco = estado_actual["historial"][-5:]
            ##Iniciamos acumuladores generales del historial completo.
            total_paquetes = 0
            total_login = 0
            total_cambios = 0
            total_salida = 0
            total_cpu = 0
            ##Iniciamos acumuladores de los ultimos 5 ciclos.
            ...
            ##Sumamos valores acumulados desde el inicio de la simulacion.
            ...
            ##Sumamos solo los ultimos 5 ciclos para mostrar el estado reciente.
            ...
            ##Evitamos dividir entre cero si por alguna razon no hay datos.
            cantidad_promedio = len(ultimos_cinco)
            if cantidad_promedio == 0:
                cantidad_promedio = 1
            ##Tambien cuidamos el total para calcular el promedio acumulado.
            cantidad_total = len(estado_actual["historial"])
            if cantidad_total == 0:
                cantidad_total = 1

            ##Guardamos totales y promedios dentro del paquete para imprimir el panel.
            ...
            ##Mandamos todos los datos calculados al modulo que imprime el panel.
            mon.imprimir_panel(
        ...
        ##Esperamos un segundo antes del siguiente ciclo para simular tiempo real.
        time.sleep(1)
